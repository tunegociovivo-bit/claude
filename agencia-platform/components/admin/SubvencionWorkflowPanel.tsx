...
export default function SubvencionWorkflowPanel({ taskId, requisitos, urlBases }: { taskId: string; requisitos: string; urlBases?: string | null }) {
  ...
  return <div className="mt-3 rounded-lg border border-indigo-200 bg-white p-3 space-y-3">
    ...
    <details className="rounded-lg border border-indigo-200 bg-indigo-50/40 p-3" open={stage === "SIGNATURE"}>
      ...
      <section className="mt-3 rounded-lg border bg-white p-3"><h4 className="text-xs font-semibold text-slate-800">Textos y dossier creados para esta solicitud</h4>{generatedText ? <pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap rounded-lg bg-slate-50 p-3 font-sans text-xs leading-5 text-slate-700">{generatedText}</pre> : <p className="mt-2 text-xs text-slate-500">Aún no se ha generado ningún texto para este expediente.</p>}</section>
    </details>
