...
type Comment = { deletedAt: string | null; id: string; authorName: string | null; authorId: string | null; authorBlockedAt: string | null; platform: string; message: string; sentiment: string; sentimentReason: string | null; aiDraft: string | null; status: string; commentCreatedAt: string; adName: string | null; feed: { clientName: string; displayName: string | null; adAccountName: string | null; campaignId: string; campaignName: string | null } };
...
export default function MetaCommentsClient() {
  ...
  return <div className="max-w-6xl mx-auto">
    ...
    {visible.length === 0 ? <div className="rounded-xl border bg-white p-10 text-center text-slate-500"><MessageSquare className="mx-auto mb-3 h-8 w-8 text-slate-300" /><div className="font-medium text-slate-700">No hay comentarios en este filtro</div><div className="mt-1 text-sm">Pulsa “Sincronizar ahora” para consultar Meta.</div></div> : <div className="space-y-4">{visible.map((item) => <article id={`meta-comment-${item.id}`} key={item.id} className={`scroll-mt-6 rounded-xl border bg-white p-5 ${item.sentiment === "negative" ? "border-rose-300" : ""}`}>
      ...
      {filter === "history" ? <div className="rounded-lg bg-amber-50 p-3 text-sm text-amber-900">{item.status === "hidden" ? "Ocultado en Meta (no eliminado)" : "Eliminado de Meta"} · {item.deletedAt ? new Date(item.deletedAt).toLocaleString("es-ES") : "Fecha no disponible"} · {item.platform === "instagram" ? "Instagram" : "Facebook"}<p className="mt-1">Conservado en el Hub para revisar las quejas. Este historial no vuelve a publicar el comentario.</p></div> : <><div className="flex items-center justify-between gap-2"><label className="text-xs font-medium text-slate-700">Borrador de respuesta (editable)</label>{item.status !== "replied" && <button onClick={() => void regenerateDraft(item)} disabled={Boolean(busy)} className="inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs font-semibold text-slate-700 disabled:opacity-50">{busy === `regenerate:${item.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />} Generar otra respuesta con IA</button>}</div><textarea value={drafts[item.id] ?? ""} onChange={(event) => setDrafts((current) => ({ ...current, [item.id]: event.target.value }))} disabled={item.status === "replied"} rows={3} className="mt-1 w-full rounded-lg border p-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200 disabled:bg-slate-50" />
