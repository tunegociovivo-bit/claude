...
import { isIrrelevantMetaComment, prepareMetaCommentReplyDraft, sanitizeMetaCommentDraft } from "@/lib/meta/comment-relevance";
...
export default function MetaCommentsClient() {
  ...
  const [importTargets, setImportTargets] = useState<Array<{ adId: string; postId: string; platform: string; returned: number; inPeriod: number; error?: string }>>([]);
  ...
  const [bulkDeleteState, setBulkDeleteState] = useState<BulkDeleteFlow>({ phase: "idle", ids: [] });
  const bulkDeleteRef = useRef<BulkDeleteFlow>({ phase: "idle", ids: [] });
  ...
  const load = useCallback(async () => {
    const response = await fetch("/api/v1/meta-comments?includeHistory=1", { cache: "no-store" });
    ...
    while (cursor) {
      const page = await fetch(`/api/v1/meta-comments?includeHistory=1&${new URLSearchParams(cursor)}`, { cache: "no-store" });
  ...
  useEffect(() => {
    if (bulkDeleteRef.current.phase === "confirming") {
      const next = bulkDeleteTransition(bulkDeleteRef.current, { type: "cancel" });
      bulkDeleteRef.current = next;
      setBulkDeleteState(next);
    }
  }, [selectedCommentIds, clientFilter, campaignFilter, filter]);
  ...
  async function reply(item: Comment) {
    const message = prepareMetaCommentReplyDraft(drafts[item.id], item.authorName);
    if (!message || !confirm(`¿Publicar esta respuesta en Meta como respuesta a ${item.authorName ?? "este usuario"}?`)) return;
    setDrafts((current) => ({ ...current, [item.id]: message }));
  ...
  async function regenerateDraft(item: Comment) {
    setBusy(`regenerate:${item.id}`); setError(null);
    try {
      const response = await fetch("/api/v1/meta-comments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "regenerate_draft", commentId: item.id }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error?.message ?? "La IA no pudo generar otra respuesta");
      setDrafts((current) => ({ ...current, [item.id]: data.draft }));
    } catch (cause: any) { setError(String(cause?.message ?? cause)); }
    finally { setBusy(null); }
  }
  ...
  async function moderate(item: Comment, action: "delete_comment" | "block_author") {
    ...
    setBusy(`${action}:${item.id}`); setError(null); setModerationResult(null);
    try {
  ...
  async function importPeriod() {
    ...
    setBusy("import"); setError(null); setImportResult(null); setImportTargets([]);
    try {
      const from = new Date(`${fromDate}T00:00:00.000`).toISOString();
      const to = new Date(`${toDate}T23:59:59.999`).toISOString();
      const { extraAdIds, extraPosts } = parseMetaImportReferences(manualAdIds);
      ...
      do {
        const response = await fetch("/api/v1/meta-comments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "sync", campaignId: selectedFeed.campaignId, clientName: selectedFeed.clientName, from, to, ...(extraAdIds.length ? { extraAdIds } : {}), ...(extraPosts.length ? { extraPosts } : {}) }) });
        const data = await response.json();
        if (!response.ok) throw new Error(data?.error?.message ?? "No se pudo importar el periodo");
        setImportTargets(data.diagnostics?.targets ?? []);
  ...
  const selectableVisible = useMemo(() => visible.filter((item) => !["deleted", "hidden"].includes(item.status)), [visible]);
  const selectedVisible = useMemo(() => selectableVisible.filter((item) => selectedCommentIds.has(item.id)), [selectableVisible, selectedCommentIds]);
  const relevantVisible = useMemo(() => selectableVisible.filter((item) => !isIrrelevantMetaComment(item.message)), [selectableVisible]);
  ...
  function requestBulkDelete() { setBulkDeleteFlow(bulkDeleteTransition(bulkDeleteRef.current, { type: "request", ids: selectedVisible.map((item) => item.id) })); }
  function confirmBulkDelete() {
    if (bulkDeleteRef.current.phase !== "confirming") return;
    const running = bulkDeleteTransition(bulkDeleteRef.current, { type: "confirm" });
    setBulkDeleteFlow(running);
    const ids = new Set(running.ids);
    void bulkAction("delete_comment", items.filter((item) => ids.has(item.id)));
  }
  ...
  async function bulkAction(action: "regenerate_draft" | "reply" | "delete_comment", confirmedTargets?: Comment[]) {
    ...
    if (action === "reply" && !confirm(`¿Publicar ${selectedVisible.filter((item) => item.status !== "replied" && prepareMetaCommentReplyDraft(drafts[item.id], item.authorName)).length} respuestas en Meta?`)) return;
    if (action === "delete_comment" && !confirmedTargets) return;
    const targets = action === "delete_comment" ? confirmedTargets! : action === "reply" ? selectedVisible.filter((item) => item.status !== "replied" && prepareMetaCommentReplyDraft(drafts[item.id], item.authorName)) : selectedVisible;
    ...
    if (action === "regenerate_draft") {
      ...
      try {
        ...
        setBulkStatus({ message: `${targets.length - failedIds.length} borradores generados de ${targets.length}.${failedIds.length ? ` ${failedIds.length} pendientes permanecen seleccionados para reintentar.` : " Listos para revisar."}` });
        if (failedIds.length) setError(lastError ?? "La IA no pudo generar todos los borradores. Puedes reintentar los comentarios pendientes.");
      } catch (cause: any) { setError(String(cause?.message ?? cause)); }
    ...
    const results = await runWithConcurrency(targets, action === "reply" ? 1 : 3, async (item) => {
      try {
        const payload = action === "reply" ? { action, commentId: item.id, message: prepareMetaCommentReplyDraft(drafts[item.id], item.authorName) } : { action, commentId: item.id };
        const response = await fetch("/api/v1/meta-comments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
        const data = await response.json();
    ...
    try {
      ...
      if (action === "delete_comment") setBulkDeleteFlow(bulkDeleteTransition(bulkDeleteRef.current, { type: "finish" }));
    }
  }
  ...
  return <div className="max-w-6xl mx-auto">
    ...
    <div className="mb-5 rounded-xl border bg-white p-4">
      ...
      <p className="mt-2 text-xs text-slate-500">Puedes pegar el ID del anuncio o el enlace de «Publicación de Facebook con comentarios» del administrador de anuncios. El enlace permite consultar también la publicación concreta de un anuncio dinámico.</p>
