...
export async function POST(req: Request, { params }: { params: { offerId: string } }) {
  ...
  try {
    ...
    const system =
      action === "review"
        ? `Eres un verificador de capturas. La fecha de HOY es ${today}. La captura DEBE ser una reseña PUBLICADA HOY en ${platform} del restaurante "${b.name}". Es válida solo si se ve que es ${platform}, es una reseña ya publicada de "${b.name}" (tolera recortes y nombres parecidos) Y su FECHA es RECIENTE, de hoy ("ahora", "hace un momento", "hace X minutos/horas", "hoy" o fecha igual a hoy). Si es de hace DÍAS, SEMANAS o MESES, NO es válida (reseña antigua o de otra persona): valid:false con confidence ALTA (0.9) y reason indicando que debe estar recién publicada hoy. Responde SOLO JSON: {"valid": boolean, "confidence": 0..1, "reason": "motivo breve en español"}.`
        : `Eres un verificador de capturas. La fecha de HOY es ${today}. La captura DEBE ser una PUBLICACIÓN en una red social (Instagram/Facebook/TikTok/X/Stories) con una FOTO que ETIQUETA o MENCIONA al restaurante "${b.name}", publicada HOY. Es válida solo si se ve que es una red social, hay foto, aparece el nombre/etiqueta de "${b.name}" Y la FECHA es RECIENTE, de hoy. Si es de hace DÍAS, SEMANAS o MESES, NO es válida: valid:false con confidence ALTA (0.9) y reason indicando que debe ser una publicación recién hecha hoy. Tolera recortes. Responde SOLO JSON: {"valid": boolean, "confidence": 0..1, "reason": "motivo breve en español"}.`;
