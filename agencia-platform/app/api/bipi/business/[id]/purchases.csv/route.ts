/**
 * GET /api/bipi/business/[id]/purchases.csv?from=YYYY-MM-DD&to=YYYY-MM-DD
 *
...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  const filename = `bipi-compras-${params.id.slice(0, 8)}-${from.toISOString().slice(0, 10)}-a-${to.toISOString().slice(0, 10)}.csv`;
