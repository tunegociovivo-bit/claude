/**
 * POST /api/bipi/ai-studio/push-copy
 *
...
- Variante C: ángulo PRUEBA SOCIAL ("lo que más piden", "los clientes Bipi están eligiendo…").
...
Tono: ${parsed.data.vibe}`;
    ...
    const data = await completeJson<{ variantes: Array<{ angulo: string; titulo: string; body: string; horarioSugerido: string }> }>({
      workspaceId: "bipi-system", // workspaceId ficticio — fallback a env OPENAI_API_KEY si no hay ai key per-workspace
    ...
    console.warn("[bipi ai-studio] fallback:", e?.message ?? e);
