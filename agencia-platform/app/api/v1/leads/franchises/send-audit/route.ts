...
export const POST = withApi({ scope: "*", rate: "admin" }, async (req, { api }) => {
  ...
  const ctaText = `Si quiere que preparemos el diagnóstico completo y el alcance del piloto, responda a este correo con “QUIERO EL DIAGNÓSTICO”.`;
  const body = `${draftBody}\n\n${ctaText}${publicAuditUrl ? `\n\nAuditoría privada y piloto: ${publicAuditUrl}` : ""}\n\ninfo@negociovivo.com · https://www.negociovivo.com`;
  const mailSubject = encodeURIComponent(`Quiero el diagnóstico de ${lead.name}`);
  const html = `<div style="font-family:Arial,sans-serif;line-height:1.55;color:#0f172a">${draftBody.split(/\n+/).map((line) => `<p>${esc(line)}</p>`).join("")}${publicAuditUrl ? `<p><a href="${esc(publicAuditUrl)}" style="display:inline-block;background:#4f46e5;color:white;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:700">Abrir auditoría privada y piloto</a></p>` : ""}<div style="margin-top:20px;padding:14px;border-radius:10px;background:#f8fafc"><strong>${esc(audit.offer.title)}</strong><br>${esc(audit.offer.pilot)}</div><div style="margin-top:20px;padding:20px;border-radius:14px;background:#111827;color:white"><strong style="font-size:18px">¿Quiere ver cómo aplicarlo a su red?</strong><p>${esc(ctaText)}</p><a href="mailto:info@negociovivo.com?subject=${mailSubject}" style="display:inline-block;background:#ffffff;color:#312e81;text-decoration:none;padding:12px 18px;border-radius:9px;font-weight:700">Solicitar diagnóstico de 15 minutos</a><span style="margin-left:14px">info@negociovivo.com</span></div><p style="font-size:12px;color:#64748b">La auditoría describe una muestra pública observada. No contiene estimaciones de ingresos ni presenta la simulación visual como una captura literal de una plataforma.</p></div>`;
