...
export const POST = withApi({ scope: "time_tracking:write" }, async (req: NextRequest, { api }) => {
  ...
  const retentionDays = Math.min(90, Math.max(1, Number(form.get("retentionDays") ?? 30)));
  ...
  await uploadBuffer({ s3Key, body: Buffer.from(await file.arrayBuffer()), contentType: file.type });
  const record = await prisma.timeTrackerScreenshot.create({ data: {
    ...
    blurred: String(form.get("blurred")) === "true",
