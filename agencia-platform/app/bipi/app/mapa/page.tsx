...
/**
 * Mapa de negocios Bipi · vista geográfica.
 *
...
export default function MapaPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch("/api/bipi/discover?limit=60");
        if (r.ok) {
    ...
    items.forEach((b) => {
      ...
      m.bindPopup(
        `<div style="font-family:system-ui;min-width:160px;">
          ...
          <a href="/bipi/n/${encodeURIComponent(b.slug)}" style="display:inline-block;margin-top:8px;padding:6px 14px;border-radius:999px;background:linear-gradient(135deg,#EC4899,#DB2777);color:white;font-size:12px;font-weight:700;text-decoration:none;">Ver y canjear</a>
        </div>`
      );
    ...
    <>
      ...
      <main className="max-w-md mx-auto pb-24">
        <div className="px-4 pt-6 pb-3">
          <span className="bipi-eyebrow">Mapa</span>
          <h1 className="text-2xl font-black tracking-tight mt-2">Negocios Bipi en el mapa</h1>
        ...
        <nav className="bipi-bottom-nav">
          ...
          <a href="/bipi/app/mapa" className="active">
            <span style={{ fontSize: 18 }}>🗺</span>
            <span>Mapa</span>
          </a>
          <a href="/bipi">
