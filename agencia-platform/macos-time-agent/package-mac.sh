...
SIGN=()
if [[ -n "${INSTALLER_IDENTITY:-}" ]]; then SIGN=(--sign "$INSTALLER_IDENTITY"); fi
pkgbuild --root "$STAGING" --component-plist "$COMPONENTS" --identifier app.negociovivo.time-agent.mac --version 1.0.0 --install-location / "${SIGN[@]}" "$OUTPUT"
