...
import { acquireRemittanceListFrame, browserValueOr, clickAfterDismissingModal, effectiveReconciliationLastAttempt, FrameRefreshRequiredError, hasVerifiedSantanderSessionText, isDirectRemittanceList, isForceReconciliationPending, isReceiptListFrame, isRemittanceListFrame, isReconciliableSepaReceipt, isSafeRemittanceConsultationLabel, isSafeRemittanceMenuLabel, isSantanderMovementRowText, missingReceiptEvidenceMessage, reconciliationRetryDecision, parseSantanderMovementText, parseSepaReceiptDebtor, parseSepaReceiptRow, parseSepaRemittanceRow, reopenRemittanceListAtPage, restoreRemittanceListFrame, runWithRefreshedFrame, selectReusableSantanderPage, shouldImportAccountMovement, shouldRunDailyReconciliation, shouldStartReconciliation } from "../src/santander/reconciliation.js";
...
async function main() {
  ...
  console.log("Conciliación de cobros:");
  {
    ...
    const restoredFrame = await restoreRemittanceListFrame(
      async () => {},
      async () => null,
      async () => ({ source: "reopened", generation: ++reopened })
    );
    ok("reabre la consulta completa cuando Atrás no restaura el listado", restoredFrame.source === "reopened" && reopened === 1);

    let reconstructed = 0;
    const acquiredList = await acquireRemittanceListFrame(
      async () => null,
      async () => ({ source: "reconstructed", generation: ++reconstructed })
    );
    ok("reconstruye filtros y cuenta si un diálogo desmonta el listado antes del siguiente recibo", acquiredList.source === "reconstructed" && reconstructed === 1);

    const visitedPages: number[] = [];
    const recoveredPage = await reopenRemittanceListAtPage(
      async () => ({ page: 0 }),
      async (_frame, pageNumber) => { visitedPages.push(pageNumber); return { page: pageNumber }; },
      3
    );
    ok("recupera también la página exacta tras reconstruir la consulta", recoveredPage.page === 3 && visitedPages.join(",") === "1,2,3");
  }
  ...
  ok("una resincronización forzada descarta el enfriamiento local del agente",
    effectiveReconciliationLastAttempt(0, null, new Date("2026-08-11T08:00:00Z"), false) === null);
  ok("el mismo reinicio de contador no provoca un bucle si ya fue consumido",
    effectiveReconciliationLastAttempt(0, null, new Date("2026-08-11T08:00:00Z"), true)?.toISOString() === "2026-08-11T08:00:00.000Z");
  ...
  ok("reconoce el listado directo que Santander conserva entre accesos",
    isDirectRemittanceList("Domiciliaciones SEPA-CORE/COR1 Remesas de un acreedor"));
  ...
  ok("solo usa el enlace interno exacto Consulta", isSafeRemittanceConsultationLabel("Consulta") && !isSafeRemittanceConsultationLabel("Consultar transferencias"));
  ok("trata una sesión caducada sin recibos como fallo recuperable", /cerró la sesión/.test(missingReceiptEvidenceMessage("AVISO Tu sesión ha caducado por inactividad")));
  ok("nunca acepta una remesa contabilizada sin detalle de recibos", /no mostró el detalle/.test(missingReceiptEvidenceMessage("Remesa contabilizada")));
  ok("acepta solo el iframe interno exacto del recibo y su remesa", isReceiptListFrame("https://empresas3.gruposantander.es/paas/rmtqry/sepa-direct-debits/receipts-list-sepa-debits", "Recibos de una remesa NÚMERO DE REMESA 0049 6611 753 000066Q FECHA DE VENCIMIENTO 09/09/2026", "00496611753000066Q"));
  ok("acepta el número de remesa compacto que devuelve Santander", isReceiptListFrame("https://empresas3.gruposantander.es/paas/rmtqry/sepa-direct-debits/receipts-list-sepa-debits", "Recibos de una remesa NÚMERO DE REMESA 00496611753000066Q FECHA DE VENCIMIENTO 09/09/2026", "00496611753000066Q"));
  ok("rechaza un iframe padre que contiene un recibo oculto", !isReceiptListFrame("https://empresas3.gruposantander.es/paas/nwe/app/portal/distribuidoras/remesas", "Recibos de una remesa 66Q Remesa 66S", "66S"));
  ok("rechaza el recibo anterior aunque el número esperado aparezca en otra zona", !isReceiptListFrame("https://empresas3.gruposantander.es/paas/rmtqry/sepa-direct-debits/receipts-list-sepa-debits", "Recibos de una remesa NÚMERO DE REMESA 0049 6611 753 000066Q FECHA DE VENCIMIENTO 09/09/2026 otra referencia 0049 6611 753 000066S", "00496611753000066S"));
  ok("acepta solo el iframe activo del listado general", isRemittanceListFrame("https://empresas3.gruposantander.es/paas/rmtqry/sepa-direct-debits/remit-list-sepa-debits", "Remesas de un acreedor") && isRemittanceListFrame("https://empresas3.gruposantander.es/paas/envrem/consulta", "Remesas de un acreedor") && !isRemittanceListFrame("https://empresas3.gruposantander.es/paas/nwe/app/portal/distribuidoras/remesas", "Remesas de un acreedor oculto"));
  ...
  ok("confirma el campo editado aunque el total de Órdenes siga antiguo", amountFieldIsConfirmed("363,00", 36300));
  ok("rechaza un campo que no registró el nuevo importe", !amountFieldIsConfirmed("544,50", 36300));
  ok("confirma el importe autorizado en el Resumen", amountSummaryIsConfirmed("363,00 EUR", 36300));
  ok("rechaza en el Resumen el importe anterior", !amountSummaryIsConfirmed("544,50 EUR", 36300));
