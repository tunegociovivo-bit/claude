  ...
  running = (async () => {
    ...
    try {
      ...
      await Promise.all(workspaces.map((w) => updateSubvencionHealth(w.id, {
        ...
        lastError: "error" in placsp ? `PLACSP: ${placsp.error}`.slice(0, 500) : null,
        ingested: ingest.upserted + ingest.curadas + placsp.upserted,
        notifications: closing.enviados + opportunities.enviados + digest.enviados,
        trigger
      }).catch(() => {})));
      return { ok: true, ingest, placsp, closing, opportunities, digest };
    } catch (error) {
