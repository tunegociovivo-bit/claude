...
export function buildWorkerCodeEmail(input: { name: string; code: string }) {
  ...
  return {
    subject: "Código para vincular tu equipo · Negocio Vivo",
    html: `<div style="font-family:Arial,sans-serif;color:#172033;line-height:1.55"><h2>Tu código de vinculación</h2><p>Hola ${name},</p><p>Introduce este código en el programa Control horario:</p><p style="font:700 20px monospace;letter-spacing:1px;background:#eef2ff;padding:14px;border-radius:9px">${code}</p><p>El código es de un solo uso y caduca en 24 horas.</p></div>`,
  };
}
