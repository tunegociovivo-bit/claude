/**
 * Helper de envío de Web Push a clientes Bipi (PWA).
 *
 * Misma mecánica que lib/push/web-push.ts (que sirve a usuarios del Hub),
 * pero apuntando a la tabla BipiPushSubscription. Se separa para que el
 * envío a clientes Bipi sea totalmente independiente.
 */

import webPush from "web-push";
import { prisma } from "@/lib/db/prisma";

let initialized = false;
function init() {
  if (initialized) return;
  if (!isBipiPushEnabled()) return;
  webPush.setVapidDetails(
    `mailto:${process.env.VAPID_CONTACT_EMAIL}`,
    process.env.VAPID_PUBLIC_KEY!,
    process.env.VAPID_PRIVATE_KEY!
  );
  initialized = true;
}

export function isBipiPushEnabled(): boolean {
  return Boolean(
    process.env.VAPID_PUBLIC_KEY &&
      process.env.VAPID_PRIVATE_KEY &&
      process.env.VAPID_CONTACT_EMAIL
  );
}

export function getBipiVapidPublicKey(): string | null {
  return process.env.VAPID_PUBLIC_KEY ?? null;
}

export async function sendPushToBipiCustomer(
  customerId: string,
  payload: {
    title: string;
    body: string;
    link?: string;
    tag?: string;
    icon?: string;
  }
): Promise<{ sent: number; removed: number }> {
  if (!isBipiPushEnabled()) return { sent: 0, removed: 0 };
  init();

  const subs = await prisma.bubuiPushSubscription.findMany({ where: { customerId } });
  let sent = 0;
  let removed = 0;

  await Promise.all(
    subs.map(async (s) => {
      try {
        await webPush.sendNotification(
          { endpoint: s.endpoint, keys: { p256dh: s.p256dh, auth: s.authKey } },
          JSON.stringify(payload)
        );
        sent++;
        // Guardamos log para analítica.
        await prisma.bubuiPushLog.create({
          data: {
            customerId,
            kind: payload.tag?.split("-")[0] ?? "generic",
            payload: payload as any
          }
        }).catch(() => {});
      } catch (e: any) {
        const status = e?.statusCode ?? 0;
        if (status === 404 || status === 410) {
          await prisma.bubuiPushSubscription.delete({ where: { id: s.id } }).catch(() => {});
          removed++;
        } else {
          console.warn("[bipi push] error:", status, e?.body ?? e?.message ?? e);
        }
      }
    })
  );
  return { sent, removed };
}
