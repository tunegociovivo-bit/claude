type ProgressRun = { createdAt: string | Date; items: Array<{ status: string }> };
...
export function getAccountancyRunProgress(run: ProgressRun, now = new Date()) {
  ...
  if (!total || completed >= total) return { completed, total, percent: total ? 100 : 0, etaMinutes: 0 };
  ...
  return { completed, total, percent, etaMinutes };
}
