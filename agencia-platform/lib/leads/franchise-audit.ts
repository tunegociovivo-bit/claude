...
export function buildFranchiseCommercialNarrative(audit: FranchiseAudit) {
  const m = audit.metrics;
  const pains: string[] = [];
  if (m.lowRatingPct > 0 || (m.ratingSpread ?? 0) >= 0.8) pains.push(`${m.lowRatingPct}% de la muestra está en zona crítica y el rango alcanza ${m.ratingSpread ?? 0} estrellas: la marca no se percibe igual en todas las ubicaciones.`);
  if (m.lowReviewsPct > 0) pains.push(`${m.lowReviewsPct}% de las unidades tiene menos de cinco reseñas: incluso una buena operación puede quedar fuera de la decisión local por falta de prueba social.`);
  if (m.reviewConcentrationPct >= 45) pains.push(`El 25% superior concentra ${m.reviewConcentrationPct}% de las reseñas: el rendimiento visible depende demasiado de pocas ubicaciones.`);
  const integrity = m.noWebsitePct + m.noPhonePct + m.domainMismatchPct + m.closedPct;
  if (integrity > 0) pains.push(`Hay fricción en los datos públicos: ${m.noWebsitePct}% sin web, ${m.noPhonePct}% sin teléfono, ${m.domainMismatchPct}% con dominio incoherente y ${m.closedPct}% no operativo.`);
  if (pains.length < 3) pains.push("Una media alta puede ocultar deterioros locales entre revisiones; sin vigilancia continua, la central suele detectarlos cuando ya afectan a reseñas, llamadas o confianza.");
  if (pains.length < 3) pains.push("Cada ficha es un punto de venta digital: si la experiencia local no sigue un estándar medible, la inversión nacional pierde consistencia en el último paso de decisión.");
  if (pains.length < 3) pains.push("La oportunidad no es solo corregir incidencias: es convertir las mejores prácticas de las unidades líderes en un sistema replicable para toda la red.");
  return {
    headline: audit.score >= 55 ? "La red presenta fugas visibles que pueden estar desviando demanda local." : audit.score >= 30 ? "La red funciona, pero existen desviaciones que debilitan la consistencia de marca." : "La red parte de una posición sólida: el reto es protegerla y convertirla en crecimiento local repetible.",
    pains: pains.slice(0, 3),
    deliverables: ["Cuadro de control por ubicación", "Alertas ante caídas y cambios", "Protocolo central de reseñas", "Corrección verificable de fichas", "Comparativa piloto vs. control", "Informe ejecutivo mensual"]
  };
}

export function buildFranchiseWeakPointSummary(audit: FranchiseAudit): string[] {
  const m = audit.metrics;
  const points: string[] = [];
  if (m.lowRatingPct > 0) points.push(`${m.lowRatingPct}% de unidades en 3,5★ o menos`);
  if ((m.ratingSpread ?? 0) >= 0.5) points.push(`${m.ratingSpread}★ de diferencia entre centros`);
  if (m.lowReviewsPct > 0) points.push(`${m.lowReviewsPct}% con visibilidad muy baja`);
  if (m.reviewConcentrationPct >= 40) points.push(`${m.reviewConcentrationPct}% de reseñas concentradas en el 25% de centros`);
  if (m.reviewsMax >= Math.max(100, Math.max(1, m.reviewsMin) * 5)) points.push(`brecha de tracción: ${m.reviewsMin}–${m.reviewsMax} reseñas`);
  if (m.noWebsitePct > 0) points.push(`${m.noWebsitePct}% de fichas sin web`);
  if (m.noPhonePct > 0) points.push(`${m.noPhonePct}% sin teléfono`);
  if (m.domainMismatchPct > 0) points.push(`${m.domainMismatchPct}% con dominio incoherente`);
  if (m.closedPct > 0) points.push(`${m.closedPct}% figura como no operativo`);
  if (points.length < 3) points.push("riesgo de deterioro local sin detección temprana");
  if (points.length < 3) points.push("mejores prácticas de centros líderes aún por sistematizar");
  return points.slice(0, 3);
}

export function buildFranchiseAuditSvg(audit: FranchiseAudit): string {
  const m = audit.metrics;
  const narrative = buildFranchiseCommercialNarrative(audit);
  ...
  const weakPointLines = buildFranchiseWeakPointSummary(audit).map((point) => `• ${point}`);
  ...
  const commercialY = locations.length ? 1180 : 1010;
  const headlineLines = wrap(narrative.headline, 88);
  const painSvg = narrative.pains.map((pain, i) => { const y = commercialY + 112 + i * 76; const lines = wrap(pain, 105); return `<circle cx="84" cy="${y}" r="20" fill="#fee2e2"/><text x="84" y="${y + 6}" text-anchor="middle" font-size="16" font-weight="900" fill="#dc2626">${i + 1}</text><text x="120" y="${y - 5}" font-size="14" font-weight="700" fill="#1e293b">${esc(lines[0] ?? "")}</text><text x="120" y="${y + 18}" font-size="14" fill="#475569">${esc(lines[1] ?? "")}</text>`; }).join("");
  const deliverableSvg = narrative.deliverables.map((item, i) => { const x = 60 + (i % 3) * 360; const y = commercialY + 366 + Math.floor(i / 3) * 48; return `<rect x="${x}" y="${y}" width="330" height="36" rx="18" fill="#ecfdf5" stroke="#a7f3d0"/><circle cx="${x + 22}" cy="${y + 18}" r="8" fill="#10b981"/><text x="${x + 22}" y="${y + 22}" text-anchor="middle" font-size="11" font-weight="900" fill="#fff">✓</text><text x="${x + 42}" y="${y + 23}" font-size="13" font-weight="700" fill="#065f46">${esc(item)}</text>`; }).join("");
  const commercialSvg = `<rect x="42" y="${commercialY - 40}" width="1116" height="500" rx="26" fill="#fff" stroke="#dbe4f0" filter="url(#shadow)"/><text x="60" y="${commercialY}" font-size="13" font-weight="850" letter-spacing="1.5" fill="#dc2626">POR QUÉ ESTO IMPORTA A LA CENTRAL</text><text x="60" y="${commercialY + 38}" font-size="22" font-weight="850" fill="#0f172a">${esc(headlineLines[0] ?? "")}</text><text x="60" y="${commercialY + 66}" font-size="22" font-weight="850" fill="#0f172a">${esc(headlineLines[1] ?? "")}</text>${painSvg}<text x="60" y="${commercialY + 340}" font-size="13" font-weight="850" letter-spacing="1.5" fill="#047857">QUÉ RECIBE LA CENTRAL CON NEGOCIO VIVO</text>${deliverableSvg}`;
  const planY = commercialY + 540;
  ...
  const ctaY = planY + 360;
  const ctaSvg = `<rect x="60" y="${ctaY}" width="1080" height="170" rx="24" fill="url(#hero)"/><text x="90" y="${ctaY + 43}" font-size="12" font-weight="850" letter-spacing="1.5" fill="#c7d2fe">SIGUIENTE PASO · SIN COMPROMISO</text><text x="90" y="${ctaY + 82}" font-size="25" font-weight="900" fill="#fff">Solicita una revisión de 15 minutos con nuestro equipo</text><text x="90" y="${ctaY + 113}" font-size="15" fill="#e0e7ff">Responde a este correo con “QUIERO EL DIAGNÓSTICO” y prepararemos el piloto para tu red.</text><rect x="90" y="${ctaY + 128}" width="270" height="30" rx="15" fill="#fff"/><text x="225" y="${ctaY + 149}" text-anchor="middle" font-size="13" font-weight="850" fill="#312e81">info@negociovivo.com</text><text x="390" y="${ctaY + 149}" font-size="13" font-weight="750" fill="#fff">www.negociovivo.com</text>`;
  const height = ctaY + 230;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="${height}" viewBox="0 0 1200 ${height}"><defs><linearGradient id="hero" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#111827"/><stop offset=".58" stop-color="#312e81"/><stop offset="1" stop-color="#4f46e5"/></linearGradient><filter id="shadow"><feDropShadow dx="0" dy="8" stdDeviation="12" flood-color="#0f172a" flood-opacity=".12"/></filter></defs><rect width="1200" height="${height}" fill="#f4f7fb"/><rect width="1200" height="300" fill="url(#hero)"/><circle cx="1080" cy="30" r="210" fill="#818cf8" opacity=".13"/><text x="60" y="52" font-size="14" font-weight="800" letter-spacing="2" fill="#c7d2fe">NEGOCIO VIVO · FRANCHISE INTELLIGENCE</text><text x="60" y="108" font-size="39" font-weight="850" fill="#fff">Auditoría de presencia local</text><text x="60" y="154" font-size="29" font-weight="750" fill="#e0e7ff">${esc(clip(audit.brand, 45))}</text><text x="60" y="184" font-size="11" font-weight="850" letter-spacing="1.4" fill="#fca5a5">PUNTOS DÉBILES A ATACAR</text><text x="60" y="208" font-size="14" font-weight="750" fill="#fff">${esc(weakPointLines[0] ?? "")}</text><text x="60" y="231" font-size="14" font-weight="750" fill="#fff">${esc(weakPointLines[1] ?? "")}</text><text x="60" y="254" font-size="14" font-weight="750" fill="#fff">${esc(weakPointLines[2] ?? "")}</text><text x="60" y="282" font-size="11" fill="#cbd5e1">Análisis independiente de ${m.sampled} fichas públicas · ${esc(new Date(audit.generatedAt).toLocaleDateString("es-ES"))}</text><rect x="930" y="82" width="210" height="126" rx="24" fill="#fff" filter="url(#shadow)"/><text x="1035" y="118" text-anchor="middle" font-size="12" font-weight="800" letter-spacing="1" fill="${riskColor}">RIESGO ${riskLabel}</text><text x="1035" y="174" text-anchor="middle" font-size="48" font-weight="900" fill="#0f172a">${audit.score}<tspan font-size="20" fill="#64748b">/100</tspan></text>${cardSvg}<text x="60" y="480" font-size="23" font-weight="850" fill="#0f172a">Radiografía de la red</text>${barSvg}<text x="630" y="460" font-size="23" font-weight="850" fill="#0f172a">Qué requiere atención</text>${findings}<text x="60" y="800" font-size="23" font-weight="850" fill="#0f172a">Establecimientos prioritarios</text><text x="1140" y="800" text-anchor="end" font-size="12" fill="#64748b">Reputación · visibilidad · integridad de ficha</text>${locationRows}${commercialSvg}<text x="60" y="${planY}" font-size="23" font-weight="850" fill="#0f172a">Plan recomendado de 60 días</text>${stepsSvg}<rect x="60" y="${planY + 214}" width="1080" height="112" rx="20" fill="#eef2ff" stroke="#c7d2fe"/><text x="88" y="${planY + 249}" font-size="12" font-weight="800" letter-spacing="1" fill="#4f46e5">PILOTO PROPUESTO</text><text x="88" y="${planY + 282}" font-size="22" font-weight="850" fill="#1e1b4b">${esc(clip(audit.offer.title, 70))}</text><text x="88" y="${planY + 309}" font-size="14" fill="#4338ca">${esc(clip(audit.offer.pilot, 125))}</text>${ctaSvg}<text x="60" y="${height - 28}" font-size="11" fill="#64748b">SIMULACIÓN VISUAL BASADA EN DATOS OBSERVADOS · No es una captura literal ni una estimación de ingresos. ${esc(audit.methodology)}</text></svg>`;
}
