...
export async function autoResolveProspectingProfiles(limit=5) {
  ...
  const prospects=await prisma.prospectingProspect.findMany({
    where:{status:"pending_resolution",resolutionStatus:{in:["unresolved","retry_pending","not_configured"]},resolutionAttempts:{lt:4},OR:[{nextResolutionAt:null},{nextResolutionAt:{lte:now}}]},
  ...
  for(const prospect of prospects){
    ...
    try{
      const keys=await resolveContactKeys(prospect.workspaceId);
      ...
      if(!domain&&prospect.companyName&&keys.hunterKey){const hit=await hunterCompanySearch({company:prospect.companyName,apiKey:keys.hunterKey,limit:10});candidates.push(...hit.people.map(p=>({name:p.name,email:p.email,title:p.position,confidence:p.confidence,source:"hunter_company"})))}
