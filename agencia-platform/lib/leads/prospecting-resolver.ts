...
export async function autoResolveProspectingProfiles(limit=5) {
  ...
  for (const prospect of prospects) {
    try {
      ...
      const wanted=nameTokens([prospect.firstName,prospect.lastName].filter(Boolean).join(' ')); const match=wanted.length>=2?candidates.find(c=>{const actual=nameTokens(c.name);return wanted.every(t=>actual.includes(t))&&actual.every(t=>wanted.includes(t))}):undefined;
      const enriched={...prospect,email:prospect.email||match?.email||null,linkedinUrl:prospect.linkedinUrl||match?.linkedin||null,jobTitle:prospect.jobTitle||match?.title||null,website:prospect.website}; const scored=scoreProspect(enriched);
